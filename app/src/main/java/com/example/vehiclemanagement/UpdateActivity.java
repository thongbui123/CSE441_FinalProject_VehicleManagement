...
public class UpdateActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.edit_interface);
        anhxa();
        addItemToSpinner();
        addEvent();
        initUI();
    ...
    private void initUI() {
        ...
        Cursor cursor = database.rawQuery("SELECT * FROM Vehicle WHERE id = ?", new String[] {id + ""});
        cursor.moveToFirst();
        String name = cursor.getString(1);
        String brand = cursor.getString(2);
        String type = cursor.getString(3);
        int dayStored = cursor.getInt(4);
        long price = doCalPriceByDay(dayStored);
        ...
        if (type.equals("Car")){
            RAD_CAR.setChecked(true);
        } else {
            RAD_MOTORBIKE.setChecked(true);
        }
        TV_DAYSTORE.setText(Integer.toString(dayStored));
        EDT_PRICE.setText(Long.toString(price));
    }
    ...
    private void addItemToSpinner() {
        ...
        ArrayAdapter adapter = new ArrayAdapter(UpdateActivity.this, android.R.layout.simple_spinner_item, ListOfBrand);
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        SPINNER.setAdapter(adapter);
        SPINNER.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                Toast.makeText(UpdateActivity.this, ListOfBrand.get(position),Toast.LENGTH_SHORT).show();
            }
    ...
    private void update(){
        ...
        database.update("Vehicle", contentValues, "ID = ?", new String[] {id + ""});
